// IPC storage bridge for the MomAI E-mails persistent worker.
//
// Accounts and notification preferences go through host-owned SQLite over IPC
// (isolated per dev mode) instead of JSON files inside the extension folder.
// Legacy files stay as a one-time import source and as the fallback for unit
// tests without an IPC channel.
...
function createIpcEmailsStorage({ send, onResponse, storageDir, timeoutMs = 30000 } = {}) {
  let seq = 0
  ...
  function call(method, args) {
    return new Promise((resolve, reject) => {
      const requestId = 'emails-' + Date.now() + '.' + seq++
