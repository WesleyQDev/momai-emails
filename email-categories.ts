...
function classifyEmailCategory(input) {
  ...
  if (from.includes('calendar') || (from.includes('google') && from.includes('event'))) return 'primary'

  const socialDomains = ['facebook', 'twitter', 'linkedin', 'instagram', 'tiktok', 'pinterest', 'reddit', 'discord', 'slack', 'whatsapp', 'telegram', 'snapchat', 'youtube', 'twitch', 'meetup', 'quora', 'tumblr']
  ...
  const updateDomains = ['notify', 'notification', 'alert', 'update', 'security', 'account', 'billing', 'support', 'service', 'info@', 'system', 'admin', 'postmaster', 'mailer-daemon', 'github', 'gitlab', 'copilot']
  ...
  const promoDomains = ['newsletter', 'marketing', 'promo', 'offers', 'deals', 'shop', 'store', 'sale', 'mailer', 'campaign', 'mailchimp', 'sendgrid', 'hubspot', 'mailgun']
